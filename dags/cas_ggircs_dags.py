...
@dag(
    dag_id=CERT_ISSUE_DAG_NAME,
    schedule=None,
    default_args=acme_renewal_args,
    doc_md=CERT_ISSUE_DAG_DOCS,
)
def issue_cert():
    @task
    def acme_issue():
        trigger_k8s_cronjob("cas-ggircs-acme-issue", ggircs_namespace)


issue_cert()


CERT_RENEWAL_DAG_DOCS = """
Renews site certificates for the GGIRCS app
"""


@dag(
    dag_id=CERT_RENEWAL_DAG_NAME,
    schedule=SCHEDULE_INTERVAL,
    default_args=acme_renewal_args,
    doc_md=CERT_RENEWAL_DAG_DOCS,
)
def acme_renewal_dag():
    @task
    def cert_renewal():
        trigger_k8s_cronjob("cas-ggircs-acme-renewal", ggircs_namespace)

    @task
    def reload_nginx():
        reload_nginx_containers("cas-ggircs", ggircs_namespace)

    cert_renewal >> reload_nginx
