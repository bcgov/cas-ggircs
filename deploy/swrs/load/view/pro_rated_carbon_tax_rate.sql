...
create or replace view ggircs_swrs_load.pro_rated_carbon_tax_rate as
    with x as (
    ...
    select x.rpd::integer as reporting_year,
           ...
           case
               when x.rpd > 2021 then (select carbon_tax_rate from ggircs_swrs_load.carbon_tax_rate_mapping where id = (select max(id) from ggircs_swrs_load.carbon_tax_rate_mapping))
               else x.rate
           end as start_rate,
