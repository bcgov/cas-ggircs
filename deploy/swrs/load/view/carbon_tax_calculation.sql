...
create or replace view ggircs_swrs_load.carbon_tax_calculation as
    with fuel as (
        ...
        from ggircs_swrs_load.fuel as _fuel
                 join ggircs_swrs_load.unit as _unit
                      on _fuel.unit_id = _unit.id
                 left join ggircs_swrs_load.emission as _emission
                      on _fuel.id = _emission.fuel_id
                      and _emission.fuel_mapping_id is not null
                 join ggircs_swrs_load.fuel_mapping as _fuel_mapping
                      on _fuel.fuel_mapping_id = _fuel_mapping.id
                      or _emission.fuel_mapping_id = _fuel_mapping.id
                 join ggircs_swrs_load.report as _report
                      on _fuel.report_id = _report.id
                 join ggircs_swrs_load.organisation as _organisation
                      on _report.id = _organisation.report_id
                 left join ggircs_swrs_load.facility as _facility
                      on _report.id = _facility.report_id
                 left join ggircs_swrs_load.naics as _naics
                      ...
                      and (select count(ghgr_import_id)
                           from ggircs_swrs_load.naics as __naics
                 ...
                 join ggircs_swrs_load.activity as _activity
                      on _unit.activity_id = _activity.id
                 join ggircs_swrs_load.pro_rated_fuel_charge as _pro_rated_fuel_charge
                 ...
                 join ggircs_swrs_load.fuel_carbon_tax_details as _fuel_carbon_tax_details
                      on _fuel_mapping.fuel_carbon_tax_details_id = _fuel_carbon_tax_details.id
                 join ggircs_swrs_load.fuel_charge as _fuel_charge
