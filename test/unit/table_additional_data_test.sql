...
-- Table ggircs_swrs_load.additional_data exists
...
select isnt_empty('select * from ggircs_swrs_load.additional_data', 'there is data in ggircs_swrs_load.additional_data');

-- FKey tests
-- Additional Data -> Activity
select set_eq(
    $$
    select distinct(activity.ghgr_import_id) from ggircs_swrs_load.additional_data
    ...
    'Foreign key activity_id in ggircs_swrs_load.additional_data references ggircs_swrs_load.activity.id'
);

-- Additional Data -> Report
select set_eq(
    $$
    select distinct(report.ghgr_import_id) from ggircs_swrs_load.additional_data
    ...
    'Foreign key report_id in ggircs_swrs_load.additional_data references ggircs_swrs_load.report.id'
);

-- Data in ggircs_swrs_transform.additional_data === data in ggircs_swrs_load.additional_data
select set_eq(
              $$
              select
                    ghgr_import_id,
              ...
              select
                    ...
                    attr_value,
                    node_value
                from ggircs_swrs_load.additional_data
                order by
                  ghgr_import_id,
                  grandparent,
                  parent,
              ...
              'data in ggircs_swrs_transform.additional_data === ggircs_swrs_load.additional_data');
