...
select has_materialized_view('ggircs_swrs', 'flat', 'ggircs_swrs_transform.flat exists as a materialized view');
...
select has_index('ggircs_swrs', 'flat', 'ggircs_flat_primary_key', 'ggircs_swrs_transform.flat has index: ggircs_flat_primary_key');
select has_index('ggircs_swrs', 'flat', 'ggircs_flat_report', 'ggircs_swrs_transform.flat has index: ggircs_flat_report');

-- Test unique indicies are defined unique
select index_is_unique('ggircs_swrs', 'flat', 'ggircs_flat_primary_key', 'ggircs_swrs_transform.flat index ggircs_flat_primary_key is unique');

-- Test columns in matview report have correct types
select col_type_is('ggircs_swrs', 'flat', 'ghgr_import_id', 'integer', 'ggircs_swrs_transform.flat.ghgr_import_id has type integer');
select col_type_is('ggircs_swrs', 'flat', 'element_id', 'bigint', 'ggircs_swrs_transform.flat.element_id has type bigint');
select col_type_is('ggircs_swrs', 'flat', 'swrs_report_id', 'numeric(1000,0)', 'ggircs_swrs_transform.flat.element_id has type bigint');
select col_type_is('ggircs_swrs', 'flat', 'class', 'character varying(10000)', 'ggircs_swrs_transform.flat.class has type varchar');
select col_type_is('ggircs_swrs', 'flat', 'attr', 'character varying(10000)', 'ggircs_swrs_transform.flat.attr has type varchar');
select col_type_is('ggircs_swrs', 'flat', 'value', 'character varying(10000)', 'ggircs_swrs_transform.flat.value has type varchar');
select col_type_is('ggircs_swrs', 'flat', 'context', 'character varying(10000)', 'ggircs_swrs_transform.flat.value has type varchar');
...
refresh materialized view ggircs_swrs_transform.flat with data;

-- test the fk relation to ghgr_import
--  Test ghgr_import_id fk relation
select results_eq(
    $$
    select ghgr_import.id from ggircs_swrs_transform.flat
    ...
    'Foreign key ghgr_import_id in ggircs_swrs_flat reference ggircs_swrs_extract.ghgr_import'
);

-- test the columnns for matview facility have been properly parsed from xml
select results_eq(
  'select element_id, swrs_report_id, class, attr, value, context from ggircs_swrs_transform.flat order by element_id',
  ...
  'ggircs_swrs_transform.flat() should return all xml nodes with values as rows');
