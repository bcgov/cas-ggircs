...
set search_path to ggircs_app, ggircs_app_private,public;
...
-- Get all table columns within schema ggircs_app that do not have a comment
...
-- Get all materialized view columns in schema ggrics_app that do not have a comment
...
with cnames as (select column_name from information_schema.columns where table_schema = 'ggircs_app')
select doesnt_match(
               col,
               '[A-Z]|\W',
...
with reserved_words as (select csv_column_fixture from csv_import_fixture),
     tnames as (select table_name from information_schema.tables where table_schema = 'ggircs_app')
select hasnt_column(
               'ggircs_app',
               tbl,
               word,
