...
with tnames as (select table_name from information_schema.tables
                    where table_schema like 'ggircs_app%'
                     and table_type != 'VIEW'
               )
select matches(
...
with tnames as (select table_name from information_schema.tables where table_schema like 'ggircs_app%')
select doesnt_match(
               tbl,
               '[A-Z]|\W',
...
schema_names as (select schema_name from information_schema.schemata where schema_name like 'ggircs_app%')
select hasnt_table(
               sch,
               res,
...
with tnames as (select table_name from information_schema.tables where table_schema like 'ggircs_app%' and table_type != 'VIEW')
select has_pk(
